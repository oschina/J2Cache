...
public class J2CacheQueryCache implements QueryCache {

    private static final CoreMessageLogger LOG = Logger.getMessageLogger(
            CoreMessageLogger.class,
            J2CacheQueryCache.class.getName()
    );
    ...
    public J2CacheQueryCache(
        ...
        if (regionNameToUse == null) {
            regionNameToUse = J2CacheQueryCache.class.getName();
        }
    ...
    public boolean put(
        ...
        for (Object aResult : result) {
            ...
            cacheRegion.put(((ManyToOneType) returnTypes[0]).getAssociatedEntityName()+":"+cacheItem, aResult);
        }
    ...
    public List get(
        ...
        for (int i = 1; i < cacheable.size(); i++) {
            try {
                if (singleResult) {
                    //TODO hibernate默认的查询缓存，只缓存标识符。对象还是通过session中load出来的
                    result.add(cacheRegion.get(((ManyToOneType) returnTypes[0]).getAssociatedEntityName() + ":" + cacheable.get(i)));
